...
### Set work dir, for example as the current folder
work.d <- setwd(".")
...
# Data can be obtained from https://cellxgene.cziscience.com/datasets. Go to:
#
#   Filters > Publication > The Tabula Sapiens Consortium* et al. (2022) Science
#
# In our scheme, we download the data for all cells as RDS. This will do it
# automatically (size is 10.4 GiB):
...
# This file contains cell compartment information for genes and is provided with
# the MatriCom app
...
# (if you are only interested in the final results and plotting, skip to SECTION 2)

# Get MatriCom data (open access). Use "Option 2: Select a sample dataset". From 
...
# be reannotated with Census. Use default MatriCom settings for both:
#
#   * Set minimum mean gene expression threshold = 1
#   * Set minimum % positive population threshold = 30%
#   * Filters: Maximize model; Use exclusion list; Remove homomeric interactions
#   * Filter by reliability score: 3
#   * Filter by communication type: Homocellular; Heterovellular
...
# folders named "TS-1.30" and "THPA-Census-1.30", respectively for TS and THPA.
...
#
# For convenience, these have been downloaded by us and are available in their
...
# Finally, we will cross-check into THPA collections for the presence of these
# same pairs.
...
# Matrisome "masterlist" has been downloaded from https://sites.google.com/uic.edu/matrisome/matrisome-annotations/homo-sapiens
...
# Download tabula sapiens metadata (https://figshare.com/articles/dataset/Tabula_Sapiens_release_1_0/14267219)
...
# Transcription factors potentially regulating the patterns. Data are from here:
#   https://www.fiserlab.org/tf2dna_db/downloads.html
...
# target table annotated from TTRUST ("Curated" file). Upstream link seems to be
# unavailable at the moment, so we provide the zipped file
...
# finally, we will export all these data into a RDS file for easier plotting in the following session (as well as for data sharing)
